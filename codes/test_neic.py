...
def to_stream(x,metadata,data):
	...
	for i,comp in enumerate(data.component_order):
		...
		tr.data = x[i,:]
		tr.stats.network   = 'WW'
		tr.stats.station   = 'NEIC'
		tr.stats.starttime = '1970-01-01T00:00:00'
...
def plot_(st,metadata,pn_picks,eq_picks):
	...
	try:
		p_pos = metadata.trace_p_arrival_sample
	...
	plt.title('NEIC: '+st[0].id+' '+str(st[0].stats.starttime))
...
for j in range(len(train)):
	try:
		...
		if len(p_pos)>2 and len(s_pos)>2:
			...
			outname = 'neic_plots/neic_'+str(j).rjust(6,'0')+'.pdf'
			plt.savefig(outname)
			plt.close()
