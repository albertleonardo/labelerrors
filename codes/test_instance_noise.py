...
data = sbd.InstanceNoise()
print(data.metadata)
print(data.metadata.columns)
print(data.metadata['station_channels'])
...
def plot_(st,metadata,pn_picks,eq_picks):
	...
	plt.title('INSTANCE noise: '+st[0].id+' '+str(st[0].stats.starttime))
...
for j in range(len(train)):
	...
	try:
		...
		if  len(p_pos)>0 and len(s_pos)>0: #or (event_type=='NO' and len(p_pos)>=2):
			print(' arrivals in noise sample')


			fig = plot_(st,data.metadata.iloc[j],p_pos,s_pos)
			
			outname = 'instance_noise/instance_'+str(j).rjust(7,'0')+'.pdf'
			plt.savefig(outname)
			plt.close()
	except Exception as e:
		print(e)
