...
def plot_(st,metadata,pn_picks,eq_picks):
	...
	try:
		...
		plt.axvline(metadata.trace_Pg_arrival_sample,c='r',label='',linewidth=3)
		plt.axvline(metadata.trace_Pn_arrival_sample,c='r',label='',linewidth=3)
		plt.axvline(metadata.trace_S1_arrival_sample,c='b',label='S label',linewidth=3)
		plt.axvline(metadata.trace_Sg_arrival_sample,c='b',label='',linewidth=3)
		plt.axvline(metadata.trace_Sn_arrival_sample,c='b',label='',linewidth=3)
	...
	plt.title('ETHZ: '+st[0].id+str(st[0].stats.starttime))
...
for j in range(len(train)):
	...
	print(p_pos,s_pos)
	print(data.metadata.iloc[j].trace_P_arrival_sample, data.metadata.iloc[j].trace_Sn_arrival_sample)
	...
	if len(p_pos)>2 and len(s_pos)>2:
		...
		outname = 'ethz_plots/ethz_'+str(j).rjust(5,'0')+'.pdf'
		plt.savefig(outname)
		plt.close('all')
