...
data = sbd.AQ2009Counts()
data.metadata['split']='train'
data.metadata.fillna('NaN',inplace=True)
#data.metadata = data.metadata.fillna('NaN')
...
def to_stream(x,metadata,data):
	...
	for i,comp in enumerate(data.component_order):
		...
		tr.stats.channel   = metadata.station_channel+comp
...
def plot_(st,metadata,pn_picks,eq_picks):
	...
	try:
		...
		#	if phase=='P':
		#		plt.axvline(all_arrivals[k],c='r',label='P label',linewidth=3)
		#	if phase=='S':
		#		plt.axvline(all_arrivals[k],c='b',label='S label',linewidth=3)
		plt.axvline(metadata.trace_p1_arrival_sample,c='r',label='P label',linewidth=3)
		if metadata.trace_p2_arrival_sample!='NaN':
			plt.axvline(metadata.trace_p2_arrival_sample,c='r',linewidth=3)
		if metadata.trace_p3_arrival_sample!='NaN':	
			plt.axvline(metadata.trace_p3_arrival_sample,c='r',linewidth=3)
		if metadata.trace_p4_arrival_sample!='NaN':
			plt.axvline(metadata.trace_p4_arrival_sample,c='r',linewidth=3)
		if metadata.trace_p5_arrival_sample!='NaN':
			plt.axvline(metadata.trace_p5_arrival_sample,c='r',linewidth=3)
		if metadata.trace_s1_arrival_sample!='NaN':
			plt.axvline(metadata.trace_s1_arrival_sample,c='b',label='S label',linewidth=3)
		if metadata.trace_s2_arrival_sample!='NaN':
			plt.axvline(metadata.trace_s2_arrival_sample,c='b',linewidth=3)
		if metadata.trace_s3_arrival_sample!='NaN':
			plt.axvline(metadata.trace_s3_arrival_sample,c='b',linewidth=3)
		if metadata.trace_s4_arrival_sample!='NaN':
			plt.axvline(metadata.trace_s4_arrival_sample,c='b',linewidth=3)
		if metadata.trace_s5_arrival_sample!='NaN':
			plt.axvline(metadata.trace_s5_arrival_sample,c='b',linewidth=3)
	...
	plt.title('AQ2009: '+st[0].id+' '+str(st[0].stats.starttime))
...
for j in range(start_index*chunk_size,(start_index*chunk_size) + chunk_size):
	...
	try:
		...
		"""
		p_names = ['trace_p1_arrival_sample','trace_p2_arrival_sample','trace_p3_arrival_sample','trace_p4_arrival_sample','trace_p5_arrival_sample']
		p_ = [data.metadata.iloc[j][p_name] for p_name in p_names if data.metadata.iloc[j][p_name]!='NaN' ]
		p_picks = list(set(p_))

		s_names = ['trace_s1_arrival_sample','trace_s2_arrival_sample','trace_s3_arrival_sample','trace_s4_arrival_sample','trace_s5_arrival_sample']
		s_ = [data.metadata.iloc[j][s_name] for s_name in s_names if data.metadata.iloc[j][s_name]!='NaN' ]
		s_picks = list(set(s_))
		print(p_,s_)
		print('P',p_picks,'S',s_picks)
		
		all_phases = p_picks + s_picks
		print(all_phases)
		# get some failure conditions to trigger plotting
		if len(p_pos)>len(all_phases) and len(s_pos)>len(all_phases) and len(all_phases)>=2: 
			print('extra arrivals')


			fig = plot_(st,data.metadata.iloc[j],p_pos,s_pos)#,all_arrivals,all_phases)
			
			outname = 'aq2009_plots/aq2009_'+str(j).rjust(7,'0')+'.pdf'
			plt.savefig(outname)
			plt.close()
