...
def plot_(st,metadata,pn_picks,eq_picks):
	...
	plt.title('STEAD: '+st[0].id+' '+str(st[0].stats.starttime))
...
for j in range(235400,1500000):
	...
	try:
		...
		print(data.metadata.iloc[j].type)
		...
		if len(p_pos)>2 and len(s_pos)>2:
			...
			outname = 'stead_plots/stead_'+str(j).rjust(7,'0')+'.pdf'
			plt.savefig(outname)
			plt.close()
	except Exception as e:
		print(e)
