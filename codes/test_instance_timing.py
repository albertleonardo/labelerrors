

# get a consensus on the arrivals of known datasets, start by getting the aq2009 or the iquiaue dataset

import numpy as np

import seisbench
import seisbench.data as sbd
import seisbench.models as sbm
import seisbench.generate as sbg

import matplotlib.pyplot as plt
plt.rcParams['font.family']='helvetica'


from obspy import Stream, Trace


# The Iquique model is problematic because the traces are 
data = sbd.InstanceCounts()
data.metadata['split']='train'

train, dev, test = data.train_dev_test()
print(train,dev,test)



train_generator = sbg.GenericGenerator(train)
dev_generator = sbg.GenericGenerator(dev)
test_generator = sbg.GenericGenerator(test)

pn_model = sbm.PhaseNet.from_pretrained("original")
eq_model = sbm.EQTransformer.from_pretrained("original")

def to_stream(x,metadata,data):
	#print(x,metadata,data)

	st = Stream()
	for i,comp in enumerate(data.component_order):
		tr      = Trace()
		tr.data = x[i,:]
		tr.stats.network   = metadata.station_network_code
		tr.stats.station   = metadata.station_code
		tr.stats.starttime = metadata.trace_start_time
		tr.stats.channel   = metadata.station_channels+comp
		tr.stats.sampling_rate = metadata.trace_sampling_rate_hz
		st += tr
	return st


# create plots for suspicious examples
def plot_(st,metadata,pn_picks,eq_picks):
	fig=plt.figure(figsize=(10,4))
	for i,tr in enumerate(st):
		plt.plot(tr.normalize().data-i,linewidth=0.5,c='k')
	try:
		plt.axvline(metadata.trace_P_arrival_sample,c='r',label='P label',linewidth=3)
		plt.axvline(metadata.trace_S_arrival_sample,c='b',label='S label',linewidth=3)
	except Exception as e:
		print(e)
	for position in pn_picks:
		plt.axvline(position,c='green',linestyle='--')
	for position in eq_picks:
		plt.axvline(position,c='orange',linestyle=':')
	#plt.xlim(0,len(st[0].data))
	plt.xlim(0,5000)

	plt.title('INSTANCE: '+st[0].id+' '+str(st[0].stats.starttime))
	plt.legend(loc='upper right')
	plt.yticks([])

	return fig

sts = []


offset=0

for j in range(len(train)):
	print(j)
	try:
		sample = train_generator[j]
		#print(sample['X'].shape)
		st    = to_stream(sample['X'],data.metadata.iloc[int(j+offset)],data)
		#sts.append(tst)

		#for j,st in enumerate(sts):
		print(st)
		# annotate and classify, compare to labels
		pn_picks = pn_model.classify(st,P_threshold=0.4,S_threshold=0.4)
		eq_picks = eq_model.classify(st,P_threshold=0.4,S_threshold=0.4)
		print(pn_picks.picks)

		# get the positions of the picks relateive to the trace tocompare to the metadata
		reftime=st[0].stats.starttime
		p_pos = [(pick.__dict__['peak_time'] - reftime)*st[0].stats.sampling_rate for pick in pn_picks.picks]
		s_pos = [(pick.__dict__['peak_time'] - reftime)*st[0].stats.sampling_rate for pick in eq_picks.picks]

		#print(pn_picks,eq_picks)
		#print(data.metadata.iloc[j])
		print(p_pos,s_pos)
		print(data.metadata.iloc[j].trace_P_arrival_sample, data.metadata.iloc[j].trace_S_arrival_sample)

		# get some failure conditions to trigger plotting
		# check that the ml picks and the labels align
		p_pn_picks = [(pick.__dict__['peak_time'] - reftime)*st[0].stats.sampling_rate for pick in pn_picks.picks if pick.__dict__['phase']=='P']
		p_eq_picks =[(pick.__dict__['peak_time'] - reftime)*st[0].stats.sampling_rate for pick in eq_picks.picks if pick.__dict__['phase']=='P']
		s_pn_picks = [(pick.__dict__['peak_time'] - reftime)*st[0].stats.sampling_rate for pick in pn_picks.picks if pick.__dict__['phase']=='S']
		s_eq_picks = [(pick.__dict__['peak_time'] - reftime)*st[0].stats.sampling_rate for pick in eq_picks.picks if pick.__dict__['phase']=='S']

		#p_pn_residuals = p_pn_picks[:1] - data.metadata.iloc[j].trace_P_arrival_sample 
		#s_pn_residuals = s_pn_picks[:1] - data.metadata.iloc[j].trace_S_arrival_sample

		#p_eq_residuals = p_eq_picks[:1] - data.metadata.iloc[j].trace_P_arrival_sample 
		#s_eq_residuals = s_eq_picks[:1] - data.metadata.iloc[j].trace_S_arrival_sample

		p_pn_residuals = p_pn_picks - data.metadata.iloc[j].trace_P_arrival_sample 
		s_pn_residuals = s_pn_picks - data.metadata.iloc[j].trace_S_arrival_sample

		p_eq_residuals = p_eq_picks - data.metadata.iloc[j].trace_P_arrival_sample 
		s_eq_residuals = s_eq_picks - data.metadata.iloc[j].trace_S_arrival_sample

		total_p_res = len(p_pn_residuals)+len(p_eq_residuals)
		total_s_res = len(s_pn_residuals)+len(s_eq_residuals)
			
		all_s_res = s_pn_residuals + s_eq_residuals
		avg_res  = np.mean(np.abs(np.asarray(all_s_res))) 
		print(s_pn_residuals,s_eq_residuals)
		print('all_s_res',all_s_res)

		if total_p_res==2 and total_s_res==2 and avg_res>200 and len(p_pn_residuals)==1 and len(s_pn_residuals)==1 and len(p_eq_residuals)==1 and len(s_eq_residuals)==1:
					
			fig = plot_(st,data.metadata.iloc[j],p_pos,s_pos)
		
			outname = 'instance_accuracy_plots/instance_'+str(int(j+offset)).rjust(7,'0')+'.pdf'
			plt.savefig(outname)
			plt.close()

	except Exception as e:
		print(e)
