...
print(data.metadata['trace_p_arrival_time'])
print(data.metadata['trace_category'])
...
def to_stream(x,metadata,data):
	...
	for i,comp in enumerate(data.component_order):
		...
		tr.stats.starttime = UT(metadata.trace_p_arrival_time) - float(metadata.trace_p_arrival_sample)/100
		print(tr)
...
def plot_(st,metadata,pn_picks,eq_picks):
	...
	plt.title('TXED: '+st[0].id+' '+str(st[0].stats.starttime))
...
for j in range(238000,len(train)):
	print(j)
	try:
		...
		if len(p_pos)>2 and len(s_pos)>2:
			...
			outname = 'txed_plots/txed_'+str(j).rjust(6,'0')+'.pdf'
			plt.savefig(outname)
			plt.close()
